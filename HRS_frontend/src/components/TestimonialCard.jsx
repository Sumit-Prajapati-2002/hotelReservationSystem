export default function TestimonialCard({ testimonial }) {
  const renderStars = (rating) => {
    const stars = [];
    for (let i = 1; i <= 5; i++) {
      if (i <= Math.floor(rating)) stars.push(<span key={i} className="text-amber-500 text-xl">★</span>);
      else if (i - rating < 1) stars.push(
        <span key={i} className="text-amber-500 text-xl relative">
          <span className="absolute overflow-hidden" style={{ width: `${(rating % 1) * 100}%` }}>★</span>
